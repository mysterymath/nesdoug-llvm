 ...
 *	, basic platformer
 *	, adding music, sfx
 *	, adding sprite zero hit (sprite 0)
 *	using neslib
 *	Doug Fraker 2018
 */
...
int main(void) {
  ...
  set_sprite_zero(); // this needs to be done before ppu_wait_nmi, before the
                     // first frame to make sure the sprite zero is in place for
                     // the split
  ...
  while (1) {
    ...
    xy_split(0, 0); // x,y
    // this split is at the bottom of the frame, should go last in the logic
    // order but, caution, not to do too much in a frame, or you will overshoot
    // the sprite zero hit, and possibly crash the game
  }
}
...
void load_room(void) {
  ...
  static const char text[] = "COINS:00  LIVES:0  LEVEL:0";
  ...
  for (char y = 0x20;; y += 0x20){ // skip the top
    for (char x = 0;; x += 0x20) {
